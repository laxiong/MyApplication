...
public class CircularScaleView extends View {
    ...
    private static final float MIN_DEGREE = 1.0F;
    ...
    public void setDegree(double first, double second) {
        this.totalMoney = (first + second) + "";
        final float dfirst = (float) (first * 100 / (first + second));
        final float dsecond = 100 - dfirst;
        progressFrist = dfirst >= dsecond ? 0 : 100;
        progressSecond = dfirst < dsecond ? 0 : 100;
        new Thread() {
            @Override
            public void run() {
                while (dfirst >= dsecond ? progressFrist < dfirst : progressSecond < dsecond) {
                    float cha = Math.min(dfirst >= dsecond ? dfirst - progressFrist : dsecond - progressSecond, MIN_DEGREE);
                    progressFrist = dfirst >= dsecond ? progressFrist + cha : progressFrist - cha;
                    progressSecond = dfirst < dsecond ? progressSecond + cha : progressSecond - cha;
                    try {
